...
use turbo_common::config_entry::ConfigOp;
...
pub async fn cmd_keypair_init(key_path: &std::path::Path, kind: &str) -> anyhow::Result<()> {
    match kind {
        "tenant" => {
            ...
            let kp = turbo_common::identity::load_or_generate_tenant_keypair(key_path)
                .with_context(|| {
        ...
        "agent" => {
            ...
            let kp = turbo_common::identity::AgentKeypair::from_signing_key(key);
...
pub async fn cmd_tenant_leave(
    ...
    println!();
    println!("Done. You may now delete ~/.turbo-tunnel/ on this machine.");
    Ok(())
}
...
pub async fn cmd_tenant_remove(
    ...
    let tenant: turbo_common::identity::TenantId = tenant_id
        .parse()
