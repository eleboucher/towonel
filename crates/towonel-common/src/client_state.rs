...
const DEFAULT_DIR: &str = ".towonel";
const STATE_FILE: &str = "state.toml";
const AGENT_KEY_FILE: &str = "agent.key";
const TENANT_KEY_FILE: &str = "tenant.key";

/// Resolved default paths anchored at the towonel state directory.
...
impl DefaultPaths {
    ...
    pub fn from_env() -> Self {
        ...
        Self {
            state_file: std::env::var_os("TOWONEL_STATE")
...
pub struct ClientState {
    /// Where the hub lives. Used as the default for `towonel-cli --hub-url`.
    ...
    /// `towonel-agent init` from the invite redemption response.
...
mod tests {
    ...
    fn roundtrip() {
        ...
        let dir = std::env::temp_dir().join(format!("towonel-state-{}", std::process::id()));
