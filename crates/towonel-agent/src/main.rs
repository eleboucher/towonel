...
struct Cli {
    ...
    /// `~/.towonel/agent.toml`, then an empty in-memory config.
    #[arg(short, long)]
...
enum Command {
    ...
    /// `~/.towonel/state.toml` + `~/.towonel/agent.toml`.
    Init {
    ...
    AddAgent {
        ...
        /// `~/.towonel/agent.key`. Fails if the file already exists
...
async fn run_agent(cli: Cli) -> anyhow::Result<()> {
    ...
    info!(services = resolved.services.len(), "towonel-agent starting");

    let service_map = Arc::new(tunnel::ServiceMap::from_config(&resolved.services).await);
    service_map.spawn_dns_refresher();

    let secret_key = towonel_common::identity::load_or_generate_secret_key(&resolved.key_path)
    ...
    if trusted_edges.is_empty() && !cli.allow_any_edge {
        anyhow::bail!(
            "no trusted_edges configured. Run `towonel-agent init --invite <token>` to \
             bootstrap, add entries manually to ~/.towonel/state.toml, or pass \
...
/// else ~/.towonel/agent.toml, else None.
