...
use towonel_common::identity::{AgentId, AgentKeypair, TenantKeypair};
...
pub async fn run(invite_str: &str, out: Option<&Path>) -> anyhow::Result<()> {
    ...
    println!("  3. Start the agent: towonel-agent");
    Ok(())
}

fn load_or_generate_tenant_keypair(path: &Path) -> anyhow::Result<TenantKeypair> {
    towonel_common::identity::load_or_generate_tenant_keypair(path)
}

fn load_or_generate_agent_keypair(path: &Path) -> anyhow::Result<AgentKeypair> {
    let signing_key = towonel_common::identity::load_or_generate_signing_key(path)?;
...
pub fn write_agent_config_template(
    ...
    content.push_str("# Auto-generated by `towonel-agent init`. The fields needed to run\n");
    content.push_str("# (keys, trusted edges, hub URL) live in ~/.towonel/state.toml.\n");
    content.push_str("# Add your services below and run `towonel-agent` to start.\n\n");
    for h in hostnames {
