//! `towonel-agent add-agent`: authorize a new agent for an existing tenant.

use std::path::{Path, PathBuf};

use anyhow::{Context, anyhow};
use towonel_common::client_state::DefaultPaths;
use towonel_common::config_entry::{ConfigOp, ConfigPayload, SignedConfigEntry};
use towonel_common::identity::{AgentId, AgentKeypair, PqPublicKey, TenantId};
...
pub async fn run(
    ...
    let tenant_kp = towonel_common::identity::load_tenant_keypair(tenant_key_path).context(
    ...
    let agent_signing_key = towonel_common::identity::load_or_generate_signing_key(&agent_key_path)
        .with_context(|| format!("failed to create agent key at {}", agent_key_path.display()))?;
    ...
    println!(
        "Next:   edit the [[services]] block and run `towonel-agent --config {}`.",
        out_path.display()
    );
    Ok(())
}
